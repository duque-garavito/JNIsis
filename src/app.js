...
import { Users, Calendar, BarChart3, UserPlus, CheckSquare, Trash2, Edit, Download } from 'lucide-react';
...
const App = () => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const attendancesSnapshot = await getDocs(query(collection(db, 'attendances'), orderBy('date', 'desc')));
  ...
  const handleAddYouth = async () => {
    ...
    try {
      ...
      setNewYouth({ name: '', age: '', phone: '', address: '', birthdate: '', group: '15-18' });
  ...
  const getChartData = () => {
    ...
    attendances.forEach(att => {
      if (!dateMap[att.date]) {
        dateMap[att.date] = { date: att.date, '15-18': 0, '19-22': 0, '23-40': 0, total: 0 };
      }
  ...
  const exportToExcel = () => {
    try {
      const headers = ['Nombre', 'Edad', 'Grupo', 'Celular', 'Fecha de Nacimiento', 'Dirección'];
      ...
      link.href = url;
      link.download = `directorio_jovenes_${new Date().toISOString().split('T')[0]}.csv`;
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-xl text-gray-700">Cargando datos desde Firebase...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="max-w-7xl mx-auto">
        <div className="bg-white rounded-2xl shadow-xl overflow-hidden">
          <div className="bg-gradient-to-r from-blue-600 to-indigo-600 p-6 text-white">
            ...
            <p className="mt-2 opacity-90">Gestiona asistencias con Firebase ☁️</p>
          </div>
          ...
          <div className="flex border-b bg-gray-50">
            <button
              ...
              className={`flex-1 py-4 px-6 font-semibold flex items-center justify-center gap-2 transition-colors ${
                activeTab === 'attendance' ? 'bg-white text-blue-600 border-b-2 border-blue-600' : 'text-gray-600 hover:bg-gray-100'
              }`}
            >
            ...
            <button
              ...
              className={`flex-1 py-4 px-6 font-semibold flex items-center justify-center gap-2 transition-colors ${
                activeTab === 'stats' ? 'bg-white text-blue-600 border-b-2 border-blue-600' : 'text-gray-600 hover:bg-gray-100'
              }`}
            >
            ...
            <button
              ...
              className={`flex-1 py-4 px-6 font-semibold flex items-center justify-center gap-2 transition-colors ${
                activeTab === 'directory' ? 'bg-white text-blue-600 border-b-2 border-blue-600' : 'text-gray-600 hover:bg-gray-100'
              }`}
            >
          ...
          <div className="p-6">
              ...
              <div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                        {groupYouths.map(youth => (
                          ...
                          >
                            ...
                            />
                            <div>
                              <div className="font-semibold text-gray-800">{youth.name}</div>
                              <div className="text-sm text-gray-600">{youth.age} años</div>
                            </div>
                          </label>
                        ))}
                      </div>
                ...
                {youths.length === 0 && (
                  <div className="text-center py-12 text-gray-500">
                    ...
                    <p className="text-sm">Ve a la pestaña "Directorio" para agregar jóvenes.</p>
                  </div>
                )}
              </div>
            )}
            ...
            {activeTab === 'stats' && (
              <div>
                <h2 className="text-2xl font-bold text-gray-800 mb-6">Estadísticas de Asistencia</h2>
                
                {attendances.length > 0 ? (
                  <>
                    <div className="mb-8">
                      <h3 className="text-xl font-semibold text-gray-700 mb-4">Asistencia por Fecha</h3>
                      <ResponsiveContainer width="100%" height={300}>
                        <LineChart data={getChartData()}>
                          ...
                          <Legend />
                          <Line type="monotone" dataKey="15-18" stroke="#3b82f6" strokeWidth={2} name="15-18 años" />
                          <Line type="monotone" dataKey="19-22" stroke="#10b981" strokeWidth={2} name="19-22 años" />
                          <Line type="monotone" dataKey="23-40" stroke="#f59e0b" strokeWidth={2} name="23-40 años" />
                    ...
                    <div className="mb-8">
                      <h3 className="text-xl font-semibold text-gray-700 mb-4">Total de Asistencias por Grupo</h3>
                      <ResponsiveContainer width="100%" height={300}>
                        <BarChart data={getGroupTotals()}>
                          <CartesianGrid strokeDasharray="3 3" />
                          <XAxis dataKey="group" label={{ value: 'Grupos de Edad', position: 'insideBottom', offset: -5 }} />
                          <YAxis label={{ value: 'Total Asistencias', angle: -90, position: 'insideLeft' }} />
                          <Tooltip />
                          <Bar dataKey="count" fill="#6366f1" name="Asistencias" />
                        </BarChart>
                      </ResponsiveContainer>
                    </div>

                    <div>
                      <h3 className="text-xl font-semibold text-gray-700 mb-4">Historial de Asistencias</h3>
                      <div className="space-y-4">
                        {attendances.map(att => (
                          <div key={att.id} className="bg-gray-50 rounded-lg p-4 border border-gray-200">
                            <div className="font-semibold text-gray-800 mb-2 flex items-center gap-2">
                              <Calendar className="w-5 h-5" />
                              {formatDate(att.date)} - {att.youths.length} jóvenes presentes
                            </div>
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-2 text-sm">
                              {groups.map(group => {
                                const groupAttendees = att.youths.filter(y => y.group === group);
                                if (groupAttendees.length === 0) return null;
                                return (
                                  <div key={group} className="bg-white rounded p-2">
                                    <div className="font-semibold text-gray-700 mb-1">Grupo {group}:</div>
                                    <div className="text-gray-600">{groupAttendees.map(y => y.name).join(', ')}</div>
                                  </div>
                                );
                              })}
                            </div>
                  ...
                  <div className="text-center py-12 text-gray-500">
                    ...
                    <p className="text-lg">No hay asistencias registradas aún.</p>
                    <p className="text-sm">Comienza tomando asistencia en la primera pestaña.</p>
                  </div>
                )}
              </div>
            )}
            ...
            {activeTab === 'directory' && (
              <div>
                <div className="flex justify-between items-center mb-6">
                  <h2 className="text-2xl font-bold text-gray-800">Directorio de Jóvenes</h2>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          if (value === '' || (value.length <= 2 && parseInt(value) >= 0)) {
                      ...
                      <input
                        ...
                        onChange={(e) => setNewYouth({ ...newYouth, birthdate: e.target.value })}
                        className="border rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-500 outline-none"
                      />
                      <input
                        type="text"
                      ...
                      <select
                        ...
                        onChange={(e) => setNewYouth({ ...newYouth, group: e.target.value })}
                        className="border rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-500 outline-none"
                      >
                        {groups.map(g => (
                          <option key={g} value={g}>Grupo {g} años</option>
                        ))}
                      </select>
                    </div>
                    <div className="flex gap-3 mt-4">
                      <button
                        onClick={editingYouth ? handleUpdateYouth : handleAddYouth}
                  ...
                  return (
                    <div key={group} className="mb-8">
                      ...
                      <div className="grid gap-4">
                        {groupYouths.map(youth => (
                          <div key={youth.id} className="bg-white border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
                            <div className="flex justify-between items-start">
                              <div className="flex-1">
                                <h4 className="font-bold text-lg text-gray-800">{youth.name}</h4>
                                <div className="grid grid-cols-2 md:grid-cols-3 gap-2 mt-2 text-sm text-gray-600">
                                  <div><span className="font-semibold">Edad:</span> {youth.age} años</div>
                                  {youth.phone && <div><span className="font-semibold">Celular:</span> {youth.phone}</div>}
                                  {youth.birthdate && <div><span className="font-semibold">Nacimiento:</span> {formatDate(youth.birthdate)}</div>}
                                  {youth.address && <div className="md:col-span-3"><span className="font-semibold">Dirección:</span> {youth.address}</div>}
                                </div>
                              </div>
                ...
                {youths.length === 0 && !showAddYouth && (
                  <div className="text-center py-12 text-gray-500">
                    <Users className="w-16 h-16 mx-auto mb-4 opacity-50" />
                    <p className="text-lg">No hay jóvenes registrados aún.</p>
                    <p className="text-sm">Haz clic en "Agregar Joven" para comenzar.</p>
                  </div>
                )}
              </div>
            )}
